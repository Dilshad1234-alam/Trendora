...
export default function CaptionGeneratorPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (dailyLimitReached) {
      ...
      setMessage({
        type: "error",
        text: "You have used all free caption generations for today.",
      });
      return;
    }
    ...
    try {
      ...
      setMessage({ type: "success", text: "Caption generated successfully." });
    } catch (error) {
      ...
      setMessage({ type: "error", text: error.message || "Unable to generate caption." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      setMessage({ type: "error", text: "Unable to copy caption." });
    }
  };

  const handleSave = async () => {
    if (!result) {
      setMessage({ type: "error", text: "Generate a caption first." });
      return;
    }
    ...
    try {
      ...
      setSaved(true);
      setMessage({ type: "success", text: "Caption saved successfully." });
    } catch (error) {
      setMessage({ type: "error", text: error.message || "Unable to save caption." });
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="mt-2 text-3xl font-black leading-tight tracking-tight text-zinc-950 sm:text-4xl">
            Generate Engaging <span className="bg-gradient-to-r from-violet-700 via-indigo-600 to-blue-600 bg-clip-text text-transparent">Captions</span>
          </h1>

          <p className="mt-3 max-w-2xl text-sm leading-relaxed text-zinc-600">
            Trendora will generate a platform-ready caption, CTA and hashtags based on your creator profile.
          </p>
        </div>
          ...
          <section className="mb-6 flex flex-col gap-4 rounded-2xl border border-violet-200 bg-violet-50 p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-600">
                Generate up to 3 captions every day.
              </p>
            </div>
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm sm:p-8 flex flex-col">
            <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-xl font-bold text-zinc-900">Generated Caption</h2>
            ...
            <div className="flex-1 flex flex-col justify-stretch">
              {loading ? (
                <div className="flex-1 flex min-h-96 flex-col items-center justify-center rounded-2xl bg-zinc-50 border border-zinc-200 text-zinc-500">
                  ...
                  <p className="font-medium">Trendora is writing your caption...</p>
                </div>
              ) : result ? (
                ...
                <div className="flex-1 flex min-h-96 flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 px-6 text-center">
                  ...
                  <p className="font-bold text-zinc-700 text-sm">No caption generated yet</p>
