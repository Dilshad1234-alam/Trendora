...
function formatWhatsAppReply({
  ...
  return [
    ...
    generatedReply.alternativeReply ? `Alternative Reply:\n${generatedReply.alternativeReply}` : "",
    generatedReply.followUpMessage ? `Follow-up Message:\n${generatedReply.followUpMessage}` : "",
  ]
    .filter(Boolean)
    .join("\n\n");
}
...
export async function POST(request) {
  try {
    ...
    if (!customerMessage) {
      return NextResponse.json({ success: false, message: "Customer message is required." }, { status: 400 });
    }
...
`;
    ...
    if (!output) {
      return NextResponse.json({ success: false, message: "AI did not generate a reply." }, { status: 503 });
    }

    const generatedReply = normalizeReply(output);

    if (!generatedReply.reply) {
      return NextResponse.json({ success: false, message: "AI returned an incomplete reply. Please try again." }, { status: 503 });
    }
    ...
    return NextResponse.json(
      {
        ...
        data: {
          ...
          dailyLimit: null,
          usedToday: null,
          remainingFreeWhatsAppReplies: null,
