...
export async function POST(request) {
  try {
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, message: "Please send valid JSON data." },
        { status: 400 }
      );
    }

    const topic = String(body.topic || "").trim().slice(0, 300);
    const platform = String(body.platform || "Facebook Ads").trim();
    const objective = String(body.objective || "Generate Leads").trim();
    ...
    if (!topic) {
      return NextResponse.json({ success: false, message: "Advertisement topic is required." }, { status: 400 });
    }
    if (!ALLOWED_PLATFORMS.includes(platform)) {
      return NextResponse.json({ success: false, message: "Invalid advertising platform." }, { status: 400 });
    }
    if (!ALLOWED_OBJECTIVES.includes(objective)) {
      return NextResponse.json({ success: false, message: "Invalid campaign objective." }, { status: 400 });
    }
    if (!ALLOWED_TONES.includes(tone)) {
      return NextResponse.json({ success: false, message: "Invalid ad-copy tone." }, { status: 400 });
    }
    if (!ALLOWED_CTAS.includes(cta)) {
      return NextResponse.json({ success: false, message: "Invalid call-to-action value." }, { status: 400 });
    }
    ...
    const audience =
      profile.targetAudience || profile.targetCustomers || profile.audience || "Potential customers";
...
`;
    ...
    try {
      ...
      console.error("Business ad-copy AI error:", aiError);
      return NextResponse.json(
        {
          success: false,
          message: aiError?.status === 429
            ? "AI request limit reached. Please try again later."
            : "AI could not generate the advertisement.",
        },
        { status: aiError?.status === 429 ? 429 : 503 }
      );
    }

    if (!output) {
      return NextResponse.json({ success: false, message: "AI did not generate ad copy." }, { status: 503 });
    }

    const adCopy = normalizeAdCopy(output);
    if (!adCopy.headline || !adCopy.primaryText) {
      return NextResponse.json({ success: false, message: "AI returned incomplete ad copy." }, { status: 503 });
    }
    ...
    const formattedAdCopy = formatAdCopy(adCopy);
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Business ad copy generated successfully.",
        data: {
    ...
    console.error("Business ad-copy generator error:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Unable to generate business ad copy.",
