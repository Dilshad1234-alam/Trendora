...
const ALLOWED_PLATFORMS = ["Instagram", "Facebook", "LinkedIn", "YouTube"];
...
export async function POST(request) {
  try {
    ...
    if (!ALLOWED_COUNTS.includes(requestedCount)) {
      return NextResponse.json({ success: false, message: "Invalid hashtag count." }, { status: 400 });
    }
    ...
    const businessCity = city || profile.city || "";
...
`;
    ...
    try {
      ...
      rawOutput = interaction.output_text?.trim();

      if (!rawOutput) {
        return NextResponse.json({ success: false, message: "AI did not generate hashtags." }, { status: 502 });
      }
    } catch (aiError) {
      console.error("Business hashtag AI error:", aiError);
      ...
      return NextResponse.json(
        {
          success: false,
          message: "AI could not generate business hashtags. Please try again.",
    ...
    const hashtagArray = [
      ...new Set(
        rawOutput
          ...
          .filter((tag) => tag.length > 1 && /^#[\p{L}\p{N}_]+$/u.test(tag))
      ),
    ].slice(0, requestedCount);

    if (hashtagArray.length === 0) {
      return NextResponse.json(
        { success: false, message: "AI returned invalid hashtags. Please try again." },
        { status: 502 }
      );
    }
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Business hashtags generated successfully.",
        data: {
    ...
    console.error("Business hashtag generator error:", error);
    return NextResponse.json(
      {
        success: false,
