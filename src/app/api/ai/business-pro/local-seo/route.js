...
function normalizeSeoOutput(output = "") {
  ...
  try {
    ...
    const categories =
      parsed.googleBusinessCategories && typeof parsed.googleBusinessCategories === "object"
        ? parsed.googleBusinessCategories
        : {};

    return {
      primaryKeyword: String(parsed.primaryKeyword || "").trim(),
      relatedKeywords: normalizeStringArray(parsed.relatedKeywords, 10),
      googleBusinessCategories: {
        primary: String(categories.primary || "").trim(),
        secondary: normalizeStringArray(categories.secondary, 3),
      },
      ...
      faqs: Array.isArray(parsed.faqs)
        ? parsed.faqs
            ...
            .slice(0, 5)
        : [],
      napChecklist: normalizeStringArray(parsed.napChecklist, 5),
      localSeoChecklist: Array.isArray(parsed.localSeoChecklist)
...
export async function POST(request) {
  try {
    ...
    const finalBusinessName = businessName.trim() || profile.businessName || "Local Business";
    const finalBusinessType = businessType.trim() || profile.businessType || "Local Business";
    const finalCity = city.trim() || profile.city || "";
    const finalState = state.trim() || profile.state || "";
    const finalCountry = country.trim() || profile.country || "India";
    const profileServices = Array.isArray(profile.services) ? profile.services.join(", ") : profile.services || "";
    const finalServices = services.trim() || profileServices || "";
    const finalAudience = audience.trim() || profile.targetCustomers || "Local customers";

    if (!finalCity) {
      return NextResponse.json(
        { success: false, message: "Business city is required to generate Local SEO." },
        { status: 400 }
      );
    }
    if (!finalServices) {
      return NextResponse.json(
        { success: false, message: "At least one business service is required." },
        { status: 400 }
      );
    }

    const location = [finalCity, finalState, finalCountry].filter(Boolean).join(", ");
...
`;
    ...
    try {
      ...
      if (!output) {
        return NextResponse.json({ success: false, message: "AI did not generate Local SEO content." }, { status: 502 });
      }
    } catch (aiError) {
      console.error("Business Local SEO AI error:", aiError);
      ...
      return NextResponse.json(
        {
          success: false,
          message: "AI could not generate the Local SEO package. Please try again.",
    ...
    const seoContent = normalizeSeoOutput(output);
    if (
      !seoContent ||
      ...
      console.error("Invalid Local SEO AI output:", output);
      return NextResponse.json(
        { success: false, message: "AI returned incomplete Local SEO data. Please try again." },
        { status: 503 }
      );
    }
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Local SEO package generated successfully.",
        data: {
    ...
    console.error("Business Local SEO error:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Unable to generate Local SEO package.",
