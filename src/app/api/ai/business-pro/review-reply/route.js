...
const ALLOWED_TONES = ["Professional", "Friendly", "Apologetic", "Warm", "Premium", "Formal"];
const ALLOWED_LANGUAGES = ["English", "Hindi", "Hinglish"];
const ALLOWED_LENGTHS = ["Short", "Medium", "Detailed"];
...
function cleanAiOutput(output = "") {
  return output.replace(/```json/gi, "").replace(/```javascript/gi, "").replace(/```/g, "").trim();
}
...
function normalizeReviewReply(output = "") {
  ...
  try {
    ...
    return {
      ...
      alternativeReply: String(parsed.alternativeReply || parsed.alternative_reply || "").trim(),
      privateFollowUp: String(parsed.privateFollowUp || parsed.private_follow_up || "").trim(),
    };
  } catch {
    return {
      reply: cleanedOutput,
...
function formatReviewReply({ platform, rating, customerName, review, generatedReply }) {
  return [
    `Platform:\n${platform}`,
    ...
    generatedReply.alternativeReply ? `Alternative Public Reply:\n${generatedReply.alternativeReply}` : "",
    generatedReply.privateFollowUp ? `Private Follow-up:\n${generatedReply.privateFollowUp}` : "",
  ]
    .filter(Boolean)
    .join("\n\n");
}
...
export async function POST(request) {
  try {
    ...
    const platform = String(body.platform || "Google").trim();
    ...
    const includeBusinessName = body.includeBusinessName !== false;
    const additionalContext = String(body.additionalContext || "").trim().slice(0, 1000);

    if (!review) {
      return NextResponse.json({ success: false, message: "Customer review is required." }, { status: 400 });
    }
    if (!validateRating(rating)) {
      return NextResponse.json({ success: false, message: "Rating must be between 1 and 5." }, { status: 400 });
    }
    if (!ALLOWED_PLATFORMS.includes(platform)) {
      return NextResponse.json({ success: false, message: "Invalid review platform." }, { status: 400 });
    }
    if (!ALLOWED_TONES.includes(tone)) {
      return NextResponse.json({ success: false, message: "Invalid review reply tone." }, { status: 400 });
    }
    if (!ALLOWED_LANGUAGES.includes(language)) {
      return NextResponse.json({ success: false, message: "Invalid review reply language." }, { status: 400 });
    }
    if (!ALLOWED_LENGTHS.includes(length)) {
      return NextResponse.json({ success: false, message: "Invalid review reply length." }, { status: 400 });
    }
    ...
    const businessName = businessProfile.businessName || businessProfile.brandName || "Our Business";
    const businessType = businessProfile.businessType || businessProfile.category || "Business";
    const location = businessProfile.city || businessProfile.location || "Not provided";
    const customerReference = customerName || "Customer";
...
Business name:
${includeBusinessName ? businessName : "Do not mention the business name"}
...
`;
    ...
    try {
      ...
      console.error("Business review-reply AI error:", aiError);
      ...
      return NextResponse.json(
        {
          success: false,
          message: "AI could not generate the review reply. Please try again.",
    ...
    if (!output) {
      return NextResponse.json({ success: false, message: "AI did not generate a review reply." }, { status: 503 });
    }

    const generatedReply = normalizeReviewReply(output);

    if (!generatedReply.reply) {
      console.error("Invalid review-reply AI output:", output);
      return NextResponse.json({ success: false, message: "AI returned an incomplete review reply. Please try again." }, { status: 503 });
    }

    const formattedReply = formatReviewReply({ platform, rating, customerName, review, generatedReply });
    ...
    return NextResponse.json(
      {
        ...
        data: {
          ...
          dailyLimit: null,
          usedToday: null,
          remainingFreeReviewReplies: null,
    ...
    console.error("Business review-reply generator error:", error);
    return NextResponse.json(
      {
        success: false,
        message: "Unable to generate review reply.",
