...
import { deleteSavedContent, getSavedContents } from "@/services/saved.api";

const contentTypes = [
  { label: "All content", value: "all" },
  { label: "Business posts", value: "business-post" },
  { label: "Captions", value: "business-caption" },
  { label: "Hashtags", value: "business-hashtag" },
  { label: "Ad copies", value: "ad-copy" },
  { label: "Local SEO", value: "local-seo" },
  { label: "Review replies", value: "review-reply" },
  { label: "WhatsApp Replies", value: "whatsapp-reply" },
];

const formatContentType = (type = "") => type.replaceAll("-", " ").replace(/\b\w/g, (l) => l.toUpperCase());
const formatDate = (date) => date ? new Intl.DateTimeFormat("en-IN", { day: "2-digit", month: "short", year: "numeric" }).format(new Date(date)) : "Unknown date";

export default function BusinessProSavedPage() {
  ...
  const loadSavedContents = useCallback(async () => {
    try {
      ...
      if (currentUser.role !== "business") return router.replace("/");
      if (currentUser.plan !== "business-pro") return router.replace("/business/dashboard");

      const response = await getSavedContents({ type: "all", search: "" });
  ...
  const filteredContents = useMemo(() => {
    ...
    return savedContents.filter((item) => {
      ...
      const searchableText = [item.title, item.content, item.type].filter(Boolean).join(" ").toLowerCase();
      const matchesSearch = !normalizedSearch || searchableText.includes(normalizedSearch);
      return matchesType && matchesSearch;
    });
  }, [savedContents, search, selectedType]);
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      ...
      setSavedContents((current) => current.filter((item) => (item.id || item._id) !== itemId));
    } catch (error) {
      setMessage(error.message || "Unable to delete saved content.");
    } finally {
      setDeletingId("");
    }
  };
  ...
  return (
    <main className="min-h-screen bg-white text-zinc-900 font-sans relative">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        {message && <div className="mb-6 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{message}</div>}
            ...
            {filteredContents.map((item) => {
              ...
              return (
                <article key={itemId} className="flex min-h-[310px] flex-col rounded-3xl border border-zinc-200 bg-white p-5 shadow-sm hover:-translate-y-1 hover:border-violet-300 hover:shadow-md">
                  ...
                  <h2 className="mt-5 line-clamp-2 text-lg font-bold text-zinc-900">{item.title || formatContentType(item.type)}</h2>
                  <p className="mt-3 line-clamp-6 whitespace-pre-wrap text-sm leading-7 text-zinc-600">{item.content}</p>
                  <div className="mt-auto pt-5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <button disabled={isDeleting} onClick={() => handleDelete(itemId)} className="inline-flex items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-600 hover:bg-red-100 disabled:opacity-50">
                        {isDeleting ? <LoaderCircle size={17} className="animate-spin" /> : <Trash2 size={17} />} Delete
                      </button>
                    </div>
                  </div>
                </article>
