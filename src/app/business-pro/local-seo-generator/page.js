        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <input type="text" placeholder="Your Business Name" className="w-full rounded-xl border p-3 outline-none focus:border-amber-500" value={formData.businessName} onChange={(e) => setFormData({...formData, businessName: e.target.value})} />
            </div>
            <div>
              <label className="mb-1 block text-sm font-bold text-zinc-700">Business Type (Optional)</label>
              <input type="text" placeholder="e.g. Plumbing Services" className="w-full rounded-xl border p-3 outline-none focus:border-amber-500" value={formData.businessType} onChange={(e) => setFormData({...formData, businessType: e.target.value})} />
            </div>
            <div>
              <label className="mb-1 block text-sm font-bold text-zinc-700">City *</label>
              <input required type="text" placeholder="e.g. New York" className="w-full rounded-xl border p-3 outline-none focus:border-amber-500" value={formData.city} onChange={(e) => setFormData({...formData, city: e.target.value})} />
            </div>
            <div>
              <label className="mb-1 block text-sm font-bold text-zinc-700">State / Province</label>
              <input type="text" placeholder="e.g. NY" className="w-full rounded-xl border p-3 outline-none focus:border-amber-500" value={formData.state} onChange={(e) => setFormData({...formData, state: e.target.value})} />
            </div>
            <div>
              <label className="mb-1 block text-sm font-bold text-zinc-700">Country</label>
              <input type="text" placeholder="e.g. USA" className="w-full rounded-xl border p-3 outline-none focus:border-amber-500" value={formData.country} onChange={(e) => setFormData({...formData, country: e.target.value})} />
            </div>
            <div>
              <label className="mb-1 block text-sm font-bold text-zinc-700">Target Audience (Optional)</label>
              <input type="text" placeholder="e.g. Homeowners" className="w-full rounded-xl border p-3 outline-none focus:border-amber-500" value={formData.audience} onChange={(e) => setFormData({...formData, audience: e.target.value})} />
            </div>
          </div>
          <div>
            <label className="mb-1 block text-sm font-bold text-zinc-700">Services *</label>
            <input required type="text" placeholder="e.g. Pipe repair, Drain cleaning" className="w-full rounded-xl border p-3 outline-none focus:border-amber-500" value={formData.services} onChange={(e) => setFormData({...formData, services: e.target.value})} />
          </div>
        ...
        {result && (
          <div className="mt-8 grid gap-6 md:grid-cols-2">
            <div className="rounded-2xl bg-zinc-50 p-6">
              ...
              <p className="mb-3 text-sm text-amber-600 font-medium">{result.primaryKeyword}</p>
              ...
              <ul className="list-disc pl-5 text-sm text-zinc-700">
                <li><span className="font-semibold text-amber-600">Primary:</span> {result.googleBusinessCategories.primary}</li>
            ...
            <div className="rounded-2xl bg-zinc-50 p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {result.relatedKeywords.map((kw, i) => (
                  <span key={i} className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700">{kw}</span>
                ))}
              </div>
            </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {result.localSeoChecklist.map((item, i) => (
                  <div key={i} className="rounded-xl border bg-white p-3">
                    <span className="mb-1 inline-block rounded bg-amber-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-amber-700">{item.priority}</span>
